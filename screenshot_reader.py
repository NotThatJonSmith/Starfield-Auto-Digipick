...
lock_radius_to_key_distance_x = 0.553 # The horizontal distance between columns of keys
lock_radius_to_key_distance_y = 0.543 # The vertical distance between rows of keys
lock_radius_to_key_radius = 0.186 # The radius of a key ring
lock_ring_stride = 0.165 # The distance between individual lock rings
key_search_grid_stride = 0.1 # The size of the grid step when looking for the selected key

class ScreenshotReader(object):

    def __init__(self, im, draw=None):
        self.im = im
    ...
    def find_first_key(self):
        ...
        brightnesses = []
        grid_step = int(self.im.height * height_to_lock_radius * key_search_grid_stride)
    ...
    def key_at(self, key_x, key_y):
        ...
        for rotation_number in range(32):
            ...
            (x0, y0) = (r*math.cos(theta), r*math.sin(theta))
            (x, y) = x0+key_x, y0+key_y
            if brightnesses[rotation_number] > mean_brightness + stdev_brightness:
